...
%  som_order_cplanes(sM,'comps',1:30,'simil',C,'pca');
...
switch proj, 
case 'SOM',
  sM2 = som_make(A,'munits',2*dim,'rect','tracking',0);
  [bm,qers] = som_bmus(sM2,A); bmi=ones(dim,1);
  hits = som_hits(sM2,A);
  %fprintf(1,'SOM size %d %d ',sM2.topol.msize(1),sM2.topol.msize(2));
  while any(hits>1), 
    mult = find(hits>1);
    ...
    while length(choices)>1,
      ...
      [mv_to,q] = som_bmus(sM2,A(mv,:),bmi(mv)); 
...
case 'ring', 
  sM2 = som_make(A,'msize',[1 max(64,2*dim)],'cyl','rect','tracking',0);
  bmus = som_bmus(sM2,A);
  Co = som_unit_coords(sM2);
  P = Co(bmus,[1 3]);   
  
case 'order',
  sM2 = som_make(A,'msize',[1 max(64,2*dim)],'tracking',0);
  bmus = som_bmus(sM2,A);
  [dummy,i] = sort(bmus); 
  [dummy,P] = sort(i);
